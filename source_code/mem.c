...
void first_fit_mem_region_finder(
    struct mem_region * head,
    struct mem_region * first,
    unsigned int size) {

    for (struct mem_region * curr = head;
         curr != NULL;
         curr = curr->next)
...
void best_fit_mem_region_finder(
    struct mem_region * head,
    struct mem_region * best,
    unsigned int size) {
    ...
    for (struct mem_region * curr = head;
        ...
        if (size <= curr->size)
            if (best == NULL || curr->size > size)
                best = curr;
}

void worst_fit_mem_region_finder(
    struct mem_region * head,
    struct mem_region * best,
    unsigned int size) {
...
void * first_fit_allocator(unsigned int size) {
    return allocator_template(size, first_fit_mem_region_finder);
}
