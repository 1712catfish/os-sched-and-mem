...
void * best_fit_allocator(unsigned int size) {
	...
	struct mem_region * smallest = current_region;
	do {
		if (size <= current_region->size) {
			found = 1;
			if(smallest->size >= current_region->size){
				smallest = current_region;
			}
			current_region = current_region->next;
		}else{
			current_region = current_region->next;
		...
		if (smallest->size == size) {
			if (smallest == free_regions) {
				...
				if (smallest->prev != NULL) {
					smallest->prev->next = current_region->next;
				}
				if (smallest->next != NULL) {
					smallest->next->prev = current_region->prev;
				}
			}
			free(smallest);
		}else{
...
void * worst_fit_allocator(unsigned int size) {
	// TODO: this is the extra function (not provided in your initial code) of worst fit allocator Implementation:
	int found = 0;
	struct mem_region * current_region = free_regions;
	struct mem_region * smallest = current_region;
	do {
